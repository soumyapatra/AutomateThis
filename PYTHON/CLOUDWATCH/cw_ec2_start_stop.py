...
def check_cwagent_tag(instance_id):
    name_tag = get_inst_tag(instance_id, "cwagent")
    if name_tag == "enable":
        return True
    else:
        return False
...
def lambda_handler(event, context):
    try:
        ...
        if eventname == "StartInstances":
            time.sleep(35)
            for item in response['instancesSet']['items']:
                if item['previousState']['name'] == "stopped" and item['currentState']['name'] == "pending":
                    ...
                    if check_bi_tag(instance_id):
                        ...
                        if check_cwagent_tag(instance_id):
                            print(put_mem_cwagent_alarm(instance_id, ENV, SNS_ARN, name))
                        else:
                        ...
                        if check_cwagent_tag(instance_id):
                            print(put_mem_cwagent_alarm(instance_id, ENV, ALERT_SNS_ARN, name))
                        else:
                            print(put_mem_alarm(instance_id, ENV, SNS_ARN, name))
                        ...
                        try:
                            ...
                            print("instance not supported. creating normal autorecover alarm. console: ", e)
                            print(put_autorecovery_check(instance_id, ENV, ALERT_SNS_ARN, name))
        ...
        msg = f'Cloudwatch Alarm Creation Failed\nIssue in Lambda function: {function_name}\nIssue: {e}\nEvent: {json_data}'
        sub = f'CloudWatch Alarm Creation Failed'
