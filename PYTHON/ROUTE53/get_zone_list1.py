...
S3_BUCKET='stage.xxxxxxxx.sonartest'
...
def upload_to_s3(folder, filename, bucket_name, key, region):
    """Upload a file to a folder in an Amazon S3 bucket."""
    key=f'route53-backup/{region}/{folder}/{key}'
...
for region in REGION:
    ...
    for zone in hosted_zones:
        ...
        upload_to_s3(zone_folder, write_zone_to_csv(zone, zone_records), S3_BUCKET, zone['Name'] + 'csv',region)
        upload_to_s3(zone_folder, write_zone_to_json(zone, zone_records), S3_BUCKET, zone['Name'] + 'json',region)
