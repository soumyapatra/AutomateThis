...
cf_dict = [{'id': 'E3337BLW6SA1PX', 'table': 'cf_cs_my11', 's3_log': 's3://xxxxxxxx-logs/structured/E3337BLW6SA1PX'}]
years = [2020]
months = [3]
days = [*range(1,32)]
...
def update_cf_athena(table_name, database, cf_s3_location, year, month, day, hour):
    ...
    query_append_data = """ALTER TABLE {} ADD PARTITION (year='{}',month='{}',day='{}',hour='{}') LOCATION '{}/{}/{}/{}/{}'""".format(table_name, year, month, day, hour, cf_s3_location, year, month, day, hour)
    response = athena_client.start_query_execution(QueryString=query_append_data,QueryExecutionContext={'Database': database},ResultConfiguration={'OutputLocation': query_output})
    print(response)
