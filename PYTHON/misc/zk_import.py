...
def get_opt_parse():
    ...
    action_group.add_option('-c', '--copy', action='store_const', const=Action.COPY,
                            dest='action',
                            help='Copy contents of source Zookeeper location to destination Zookeeper location.')
    ...
    action_group.add_option('-i', '--import', action='store_const', const=Action.IMPORT,
                            dest='action',
                            help='Read contents of target JSON file and write data to destination Zookeeper location.')
    parser.add_option_group(action_group)
    parser.set_defaults(action=Action.PRINT)

    # Resolution
    resolution_group = OptionGroup(parser,
...
class ZkTreeUtil(object):
    ...
    def process_znode_write_to_zk(self, znode, dest_zk_client, dest_zk_path, resolve):
        ...
        if (dest_znode_path == "/zookeeper/config"):
            self.logger.debug("Skipping ZNode at /zookeeper/config as don't have permission to update")
            return
        elif isinstance(znode.data, dict):
            znode_val = json.dumps(znode.data)
        elif isinstance(znode.data, list):
            znode_val = json.dumps(znode.data)
        else:
            znode_val = str(znode.data)

        if dest_zk_client.exists(dest_znode_path):
            if resolve == Resolve.OVERWRITE:
                self.logger.debug('ZNode at %s already exists. Overwriting data due to --overwrite' % dest_znode_path)
                dest_zk_client.set(dest_znode_path, znode_val)
            ...
            dest_zk_client.create(dest_znode_path, znode_val, makepath=True)

    def process_znode_write_dict(self, znode, znode_dict):
        try:
            val = json.loads(str(znode.data))
        except ValueError:
            val = znode.data
        znode_dict[znode.path] = val
    ...
    def run_import(self, source_file, dest_zk, dest_zk_path, resolve):
        ...
        with open(source_file, "rb") as f:
            znode_dict = json.loads(f.read().decode("UTF-8"))
            for (path, znode_val) in znode_dict.items():
                znode = ZNode(path, znode_val)
    ...
    def run_export(self, source_zk, source_zk_path, dest_file):
        ...
        f = open(dest_file, 'w')
        f.write(json.dumps(znode_dict, sort_keys=True, ensure_ascii=False, indent=4 * ' '))
