...
def get_tag_val(tag_key):
    resp = []
    tag_list = []
    response = client.describe_tags(Filters=[{'Name': 'key', 'Values': [tag_key]}], MaxResults=123)
    resp.extend(response["Tags"])
    while "NextToken" in response:
        response = client.describe_tags(Filters=[{'Name': 'key', 'Values': [tag_key]}], MaxResults=123,
                                        NextToken=response['NextToken'])
        resp.extend(response["Tags"])
    for tag in resp:
        if tag["ResourceType"] == "instance" and tag["Key"] == tag_key:
...
def getTotalReport(tagKey="", tagVal="", start_date="", end_date=""):
    total_df = []
    results = []
    days = 3
    count = 0
    ...
    while count < days:
        results.extend(getCostNUsage(start_date - timedelta(days=count), end_date - timedelta(days=count), Filter))
        count += 1
    for v in results:
...
class CostExplorer:
    def __init__(self, CurrentMonth=False):
        ...
        self.end = date.today() - timedelta(days=int(DAYS - 1))
    ...
    def addEc2Report(self, Name="Default", GroupBy=[{"Type": "DIMENSION", "Key": "SERVICE"}, ],
        ...
        try:
            df = pd.DataFrame(rows)
            df.set_index(TagKey, inplace=True)
            df = df.fillna(0.0)
            ...
            self.reports.append({'Name': Name, 'Data': df, 'Type': type, 'length': rows_length,
                                 "total_df": getTotalReport(TagKey, TagVal, self.start, self.end)})
            return df
        except KeyError:
            df = pd.DataFrame(rows)
            # df.set_index(TagKey, inplace=True)
        ...
        for report in self.reports:
            ...
            try:
                if report['Type'] == 'chart':
                    ...
                    for i in report["total_df"]:
                        i["df"].to_excel(writer, sheet_name=report['Name'], startrow=report["length"] + 5,
                                         startcol=0)
...
def main_handler(event=None, context=None):
    # tag_val = ["ee_k8s_pt", "rc_stage_gr"]
    tag_val = get_tag_val("billing_unit")

    costexplorer = CostExplorer(CurrentMonth=False)
    for tag in tag_val:
        costexplorer.addEc2Report(Name=f"{tag}", GroupBy=[{"Type": "DIMENSION", "Key": "SERVICE"}],
                                  Style='Total',
                                  TagKey='billing_unit', TagVal=[tag])
        # costexplorer.getTotalReport(tagKey="billing_unit", tagVal=[tag])
    costexplorer.generateExcel(reportName="PT Applications (Stage Account)")
