...
def get_instance_tag(region, search_tag, search_tag_val_list, target_tag):
    ...
    for instance in instance_iterator:
        tags = instance.tags
        for tag in tags:
            if tag["Key"] == search_tag and tag["Value"] == search_tag_val_list:
                for tag in tags:
    ...
    def addEc2Report(self, Name="Default", GroupBy=[{"Type": "DIMENSION", "Key": "SERVICE"}, ],
        ...
        for tag in tags:
            ...
            Filter = {"And": [{"Tags": {"Key": TagKey, "Values": [tag]}},
                              {"Tags": {"Key": "billing_unit", "Values": [TagVal]}},
...
def main_handler(event=None, context=None):
    try:
        ...
        for tag in tag_val:
            ...
            days = 3
            count = 0
            report_name = tag.split("_")[-2].upper()
            while count < days:
                report_date = date.today() - timedelta(days=count + 1)
                costexplorer.addEc2Report(Name=f"Production GR - {report_date}",
                                          GroupBy=[{"Type": "DIMENSION", "Key": "SERVICE"}],
                                          Style='Total',
                                          TagKey='Application', TagVal=tag, days_old=count)
                count += 1
            costexplorer.getTotalReport(tagKey="billing_unit", tagVal=tag)
            costexplorer.getTotalReport(tagKey="billing_unit", tagVal=tag)
            costexplorer.generateExcel(reportName=f"GR Applications(DR Account) - {report_name}")
    except Exception as e:
