...
class CostExplorer:
    ...
    def addEc2Report(self, Name="Default", GroupBy=[{"Type": "DIMENSION", "Key": "SERVICE"}, ],
        ...
        type = 'chart'
        tags = getInstanceTag(EC2_REGION, "billing_unit", TagVal, "billing_subunit")
        print("Tag list:", tags)
        rows = []
        for tag in tags:
            results = []
        ...
        df.set_index("billing_subunit", inplace=True)
        df = df.fillna(0.0)
...
def main_handler(event=None, context=None):
    ...
    costexplorer.addEc2Report(Name="Production BI", GroupBy=[{"Type": "DIMENSION", "Key": "SERVICE"}],
                              Style='Total',
                              TagKey='billing_subunit', TagVal=tag_val)
    costexplorer.getTotalReport(tagKey="billing_unit", tagVal=tag_val)
    # costexplorer.getTotalReport(tagKey="billing_unit", tagVal=tag_val)
    costexplorer.generateExcel(reportName="Prod BI EMR (BI Prod Account)")
