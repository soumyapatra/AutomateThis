terraform {
  source = "${get_parent_terragrunt_dir()}/../modules//ingress_controller"
}
...
dependency "eks" {
  ...
  mock_outputs = {
    ...
    name = "fake-name"
    oidc = {"url": "oidc-url", "arn": "oidc-arn"}
  }
}
dependency "ng" {
  config_path = "../ng"
...
inputs = {
  ...
  oidc_provider = dependency.eks.outputs.oidc["url"]
  oidc_arn = dependency.eks.outputs.oidc["arn"]
